...
// Import shared concepts from the Stepanov concepts layer.
// These are the canonical definitions used across all libraries.
using stepanov::Arithmetic;
...
//
// For a symmetric positive-definite matrix A, computes the lower triangular
// matrix L such that A = L * L^T. This is the standard algorithm for
// sampling from multivariate normal distributions.
//
// Precondition: A must be symmetric positive-definite.
// If A is not positive-definite, the algorithm will encounter a negative
// value under the square root and throw.
//
// Algorithm: column-by-column computation
//   L(j,j) = sqrt(A(j,j) - Σ_{k<j} L(j,k)²)
//   L(i,j) = (A(i,j) - Σ_{k<j} L(i,k)*L(j,k)) / L(j,j)   for i > j
template <Matrix M>
[[nodiscard]] auto cholesky(const M& A) -> matrix<typename M::scalar_type> {
    if (A.rows() != A.cols()) {
    ...
    for (std::size_t j = 0; j < n; ++j) {
        // Diagonal element: L(j,j) = sqrt(A(j,j) - Σ L(j,k)²)
        T sum_sq{0};
