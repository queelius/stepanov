...
/**
 * A type T equipped with a binary operator op : (T,T) -> T is a monoid if:
 * 
 *     (1) Associativity: For all a,b,c in T,
 *             op(a,op(b,c)) == op(op(a,b),c).
 *     (2) Identity element: there is a (unique) element e in T such that
 *             op(x,e) == x && op(e,x) = x.
 * 
 * 
 */
bool is_monoid(abelian_op const & f)
{
    auto e = identity(f);
    if (!e || !is_associative())
        return false;
...
/**
 * A type T with a binary operator t : (T,T) -> T is a monoid if:
 * 
 *     (1) There is a (unique) identity element e under t,
 *             t(x,e) == x && t(e,x) = x.
 * 
 *     (2) For each x in T there is a coresponding element x' in T such that
 *             t(x,x') = e
 * 
 *     (3) 
 */
 ...
 */
template <unsigned int K>
std::optional<nat<K>> identity(abelian_op<K> const & f)
{
